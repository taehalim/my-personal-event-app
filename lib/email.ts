...
export async function sendRegistrationEmail(type: 'registration_approved'|'registration_pending'|'registration_rejected'|'registration_cancelled', event: LamaEvent, registration: {name: string; email: string; cancelToken?: string}, reason?: string) {
  ...
  const labels: Record<string, [string,string]> = { registration_approved: ['[참가 신청 완료]', '참가 신청이 완료되었습니다.'], registration_pending: ['[신청 접수]', '신청이 접수되었습니다. 관리자 승인 후 참가가 확정됩니다.'], registration_rejected: ['[신청 결과 안내]', '아쉽게도 이번 행사 참가 신청이 승인되지 않았습니다.'], registration_cancelled: ['[참가 취소 완료]', reason ?? '참가 신청이 취소되었습니다.'] };

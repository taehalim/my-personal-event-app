...
- 카드 전체 클릭은 공개 slug로 이동한다.
- 카드에는 시간, 제목, 한 줄 말줄임 장소, 참가자 정보, 정사각형 대표 이미지만 둔다. 높이는 메타데이터 길이에 따라 달라지지 않는다.
- 지난 이벤트는 카드 전체 대비를 낮춘다. 예정 뱃지는 붙이지 않으며 진행 중일 때만 절제된 accent를 허용한다.
...
- 공개 목록과 동일한 제목, 탭, 날짜 rail, 카드 시각 계층을 쓴다.

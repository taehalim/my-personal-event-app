...
async function requireOwnedEvent(eventId: string, userId: string) {
  ...
  const { data } = await admin.from('events').select('id').eq('id', parsed.data).eq('created_by', userId).maybeSingle();
...
export async function POST(request: Request) {
  ...
  if (!(await requireOwnedEvent(eventId, user.id))) return NextResponse.json({ error: { code: 'NOT_FOUND', message: '이벤트를 찾을 수 없습니다.' } }, { status: 404 });
...
export async function DELETE(request: Request) {
  ...
  if (!(await requireOwnedEvent(eventId, user.id)) || !uploadPathSchema.safeParse(path).success || !path.startsWith(`${eventId}/`)) return NextResponse.json({ error: { code: 'INVALID_INPUT', message: '이미지 경로를 확인해주세요.' } }, { status: 400 });

...
export default function EventForm({ event, fields, hostName: suppliedHostName }: { event?: LamaEvent; fields?: ExistingField[]; hostName?: string }) {
  ...
  const [hostName] = useState(event?.host_name ?? suppliedHostName ?? '나');
  ...
  const isDarkCanvas = isDarkEventBackground(selectedBackground);
  ...
  useEffect(() => {
    ...
    document.body.dataset.eventEditorTone = isDarkCanvas ? 'dark' : 'light';
    return () => {
      delete document.documentElement.dataset.eventEditorBackground;
      delete document.body.dataset.eventEditorBackground;
      delete document.body.dataset.eventEditorTone;
    };
  }, [isDarkCanvas, selectedBackground]);
